...
function resolveAgentId(providedAgentId?: string): string | undefined {
...
export async function memoryAdd(ctx: ToolContext, input: any) {
...
export async function memoryWrite(ctx: ToolContext, input: any) {
  ...
  const mergedMetadata = {
    ...(input.metadata || {}),
    source: (input.metadata && (input.metadata as any).source) || "mcp_memory_write"
  };
  ...
  if (result && typeof result === 'object' && 'memory_ids' in result && Array.isArray((result as any).memory_ids) && (result as any).memory_ids.length > 0) {
...
export async function memoryRecall(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
  ...
  if (resolvedAgentId && result && typeof result === 'object' && 'memories' in result) {
    ...
    if (!memories || memories.length === 0) {
      ...
      if (hint) {
        return {
          content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) + hint }],
        };
      }
    }
  }
...
export async function memorySearch(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
...
export async function memoryList(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
...
export async function memoryDelete(ctx: ToolContext, input: any) {
  const result = await api({
    apiKey: ctx.apiKey,
    method: "DELETE",
...
export async function listAgents(ctx: ToolContext) {
  const result = await api({
    apiKey: ctx.apiKey,
    method: "GET",
    path: "/agents",
...
export async function memoryHistory(ctx: ToolContext, input: any) {
  const result = await api({
    apiKey: ctx.apiKey,
...
export async function memoryGraphTraverse(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
  if (!resolvedAgentId) throw new Error("agent_id is required for memory_graph_traverse");
...
export async function memoryEntities(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
  if (!resolvedAgentId) throw new Error("agent_id is required for memory_entities");
...
export async function memoryByEntity(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
  if (!resolvedAgentId) throw new Error("agent_id is required for memory_by_entity");
...
export async function importDocument(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
...
export async function importConversation(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
...
export async function memoryFeedback(ctx: ToolContext, input: any) {
  const resolvedAgentId = resolveAgentId(input.agent_id);
