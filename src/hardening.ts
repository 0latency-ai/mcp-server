...
setInterval(() => {
  ...
  for (const [hash, entry] of dedupMap.entries()) {
    if (now > entry.expiresAt) dedupMap.delete(hash);
  }
}, 60000);
...
export function checkDedup(content: string, agentId: string): string | null {
  const hash = crypto.createHash("sha256").update(content + agentId).digest("hex");
  const now = Date.now();
  const entry = dedupMap.get(hash);
  if (entry && now < entry.expiresAt) return entry.memoryId;
  return null;
}

export function recordDedup(content: string, agentId: string, memoryId: string): void {
  const hash = crypto.createHash("sha256").update(content + agentId).digest("hex");
 ...
 */
export async function runActiveProfiler(
  apiKey: string,
  agentId: string,
  humanMessage: string,
  sessionKey?: string
...
export function formatSentinelWarning(sentinel: SentinelResponse): string {
  if (!sentinel?.detected?.length) return "";
  const lines = [
    "",
...
export function appendSentinelWarning(baseText: string, apiResult: any): string {
  if (apiResult?.sentinel) {
    const warning = formatSentinelWarning(apiResult.sentinel);
    if (warning) return baseText + warning;
  }
  return baseText;
}
