...
function extractApiKey(req) {
    ...
    if (auth?.startsWith("Bearer ")) {
        try {
            const decoded = jwt.verify(auth.replace("Bearer ", ""), process.env.JWT_SECRET || "changeme", { issuer: "https://mcp.0latency.ai" });
...
app.post("/token", express.urlencoded({ extended: true }), (req, res) => {
    ...
    const access_token = jwt.sign({ api_key: authCode.api_key }, process.env.JWT_SECRET || "changeme", { expiresIn: "1h", issuer: "https://mcp.0latency.ai" });
